...
const updateCurrentDate = () => {
    const updateDate = () => {
        ...
        // Format date
        const formattedDate = `${time} - ${date.toLocaleString('en-US', { weekday: 'long' })}, ${date.getDate()} ${date.toLocaleString('en-US', { month: 'short' })} ${date.getFullYear()}`;
        dateVal.forEach(x => {
...
const updateWeatherUI = async () => {
    ...
    temperatureImg.setAttribute("src", `assets/weather/${getTemperatureImg(id)}.svg`)
...
const updateForecastUI = async () => {
    ...
    forecastData.list.forEach(forecastWeather => {
        if (n != 0 && forecastWeather.dt_txt.includes(timeTaken) && !forecastWeather.dt_txt.includes(todayDate)) {
        n--;
        ...
        const formattedDate = `${actualDate.getDate()} ${actualDate.toLocaleString('en-US', { month: 'short' })}`;

            forecastSection.innerHTML += `
            <div class="forecast-container">
                    <img src="assets/weather/${getTemperatureImg(id)}.svg" alt="${main}" class="temperature-img small">
                    <div class="day-info">
                        <p class="day-date">${formattedDate}</p>
                        <p class="day-temperature-info"><span class="day-temperature">${temp.toFixed(1)}</span>° C</p>
                    </div>
                </div>
            `
        }
